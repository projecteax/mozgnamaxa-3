...
interface ProgressPage11Props {
  onContinue: () => void
}

export default function ProgressPage11({ onContinue }: ProgressPage11Props) {
  ...
  const handleDalejClick = () => {
    console.log("DALEJ button clicked from progress page 11 - Redirecting based on user login status")
    onContinue()
  }
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center justify-center px-12"
      ...
      <div className="grid grid-cols-6 gap-6 mb-16">
        ...
        {/* Tenth box - active with medal 10 - enlarged from w-24 h-24 to w-48 h-48 */}
        <div className="relative w-48 h-48">
          <Image src="/images/white_box_medium.svg" alt="Active box" fill className="object-contain" />
          <div className="absolute inset-0 flex items-center justify-center">
            {/* Medal icon enlarged from w-12 h-12 to w-24 h-24 */}
            <div className="relative w-24 h-24">
              <Image src={`/images/medal_10${medalSuffix}.svg`} alt="Medal 10" fill className="object-contain" />
            </div>
          </div>
        </div>

        {/* Eleventh box - active with medal 11 - enlarged from w-24 h-24 to w-48 h-48 */}
        <div className="relative w-48 h-48">
          <Image src="/images/white_box_medium.svg" alt="Active box" fill className="object-contain" />
          <div className="absolute inset-0 flex items-center justify-center">
            {/* Medal icon enlarged from w-12 h-12 to w-24 h-24 */}
            <div className="relative w-24 h-24">
              <Image src={`/images/medal_11${medalSuffix}.svg`} alt="Medal 11" fill className="object-contain" />
            </div>
          </div>
        </div>

        {/* Remaining 1 box - greyed out - enlarged from w-24 h-24 to w-48 h-48 */}
        <div className="relative w-48 h-48">
